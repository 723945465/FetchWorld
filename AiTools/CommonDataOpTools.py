...
def query_timescope_hismsg(start_time, end_time,topic = ''):
    try:
        # 连接到MySQL数据库
        connection = mysql.connector.connect(host=db_host, database=db_databasename, user=db_user, password=db_password, charset = charset)
        if connection.is_connected():
            cursor =  cursor = connection.cursor(dictionary=True)  # 使用字典游标，方便转换为 JSON
            # 定义SQL查询
            if topic is None or len(topic) == 0:
               sql = """
                    ...
                    AND info_ready_for_analysis = 'yes' 
               ...
               cursor.execute(sql, (start_time, end_time))
                ...
                sql = """
                     ...
                     AND (info_bad_for_analysis != 'bad' OR info_bad_for_analysis IS NULL)
                     AND LENGTH(info_match_topic) >= 2
                     AND info_match_topic LIKE CONCAT('%%', %s, '%%')
                     ORDER BY create_time DESC;
                     """
            # 执行查询
                cursor.execute(sql, (start_time, end_time, topic))
            ...
            if sql_result and len(sql_result) > 0:
                # 解析JSON数据
                json_result = json.dumps(sql_result, default=str, ensure_ascii=False)
                # print(len(res_json))
                # print(res_json)
                return json_result
            else:
...
def query_today_hismsg(Topic):
    ...
    return query_timescope_hismsg(start_time, end_time,Topic)


def query_halfday_hismsg():
    try:
        # 连接到MySQL数据库
        connection = mysql.connector.connect(host=db_host, database=db_databasename, user=db_user, password=db_password, charset = charset)
        if connection.is_connected():
            ...
            sql = """
                    ...
                    WHERE create_time BETWEEN 
                        ...
                        AND 
                        DATE_FORMAT(NOW(), '%Y-%m-%d 11:59:59')
                        OR create_time BETWEEN 
                        DATE_FORMAT(NOW(), '%Y-%m-%d 12:00:00') 
                        AND 
